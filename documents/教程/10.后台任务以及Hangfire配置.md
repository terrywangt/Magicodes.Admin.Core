...
AbpZero框架已经集成了hangfire，但它默认是关闭的，我们可以在运行站点下的Startup.cs文件中把这行代码注释取消就行了（如果没有那就手动进行添加），代码如下：
 ...
 一般在启用执行的前面加上你得执行判断条件，根据你的任务调度需求进行条件的限定： 
    //仅在后台服务启用
 if (!_appConfiguration["Abp:Hangfire:IsEnabled"].IsNullOrEmpty() && Convert.ToBoolean(_appConfiguration["Abp:Hangfire:IsEnabled"]))然后把判断参数我们写在框架的配置文件中 这样我们就能根据需求更轻松的更改我们的限制条件参数。
