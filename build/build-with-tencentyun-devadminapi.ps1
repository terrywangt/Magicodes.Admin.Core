...
param(
    ...
    $replaceHostConfigFile="appsettings.devadminapi.json",
    $replaceAppHostConfigFile="appsettings.devappapi.json",
...
function PublishWebHostFolder {
    ...
    if(![String]::IsNullOrEmpty($replaceHostConfigFile))
    {
        $configFilePath=Join-Path $buildFolder $replaceHostConfigFile;
        $beReplaceConfigFilePath=Join-Path $hostOutputPath $hostConfigFile;
        if([io.File]::Exists($configFilePath))
        {

            LogDebug "configFilePath:$configFilePath"
            LogDebug "beReplaceConfigFilePath:$beReplaceConfigFilePath"

            Copy-Item $configFilePath -Destination $beReplaceConfigFilePath
...
function PublishAppHostFolder {
    ...
    if(![String]::IsNullOrEmpty($replaceAppHostConfigFile))
    {
        $configFilePath=Join-Path $buildFolder $replaceAppHostConfigFile;
        $beReplaceConfigFilePath=Join-Path $hostOutputPath $hostConfigFile;
